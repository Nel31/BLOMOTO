...
export default function GarageInvoicesPage() {
  ...
  const loadQuotes = async () => {
    try {
      ...
      // Charger uniquement les devis acceptés qui n'ont pas encore de facture
      const acceptedQuotes = (res.data.quotes || []).filter((q: any) => q.status === 'accepted');
  ...
  const handleCreateFromQuote = async (quoteId: string) => {
    try {
      const res = await api.post(`/invoices/from-quote/${quoteId}`, {
  ...
  const handleSendInvoice = async (sendViaChat: boolean, sendViaEmail: boolean) => {
  ...
  const handleMarkAsPaid = async (paidAmount: number, paymentMethod: string) => {
    if (!selectedInvoice) return;
    try {
      await api.put(`/invoices/${selectedInvoice._id}/mark-paid`, {
        paidAmount,
        paymentMethod,
      });
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    if (field === 'quantity' || field === 'unitPrice') {
      (updatedItem as any).total = updatedItem.quantity * updatedItem.unitPrice;
    }
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold" style={{ color: 'var(--color-noir)' }}>Factures</h1>
        </div>
        <button
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4" style={{ color: 'var(--color-noir)' }}>Nouvelle facture</h2>
            
            {quotes.length > 0 && (
              <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                <p className="font-medium mb-2" style={{ color: 'var(--color-noir-700)' }}>
      ...
      {showSendModal && selectedInvoice && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-2xl font-bold mb-4" style={{ color: 'var(--color-noir)' }}>Envoyer la facture</h2>
            <p className="mb-4" style={{ color: 'var(--color-noir-700)' }}>
              Comment souhaitez-vous envoyer la facture {selectedInvoice.invoiceNumber} ?
            </p>
            <div className="space-y-4">
            ...
            <div className="flex justify-end gap-4 mt-6">
              ...
              <button
                onClick={() => {
                  ...
                  handleSendInvoice(sendChat, sendEmail);
                }}
                className="px-6 py-2 rounded-lg font-semibold text-white"
                style={{ backgroundColor: 'var(--color-rouge-600)' }}
              >
            ...
            <div className="flex justify-end gap-4 mt-6">
              ...
              <button
                onClick={() => {
                  ...
                  handleMarkAsPaid(paidAmount, paymentMethod);
                }}
                className="px-6 py-2 rounded-lg font-semibold text-white"
                style={{ backgroundColor: 'var(--color-rouge-600)' }}
              >
